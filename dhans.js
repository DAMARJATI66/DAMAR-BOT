require('./config.js')^M
let { WAConnection: _WAConnection } = require('@adiwajshing/baileys')^M
...
^M
global.prefix = new RegExp('^[' + (opts['prefix'] || '.!.^#$?¥\/%+×1¥07*-,@ZQ').replace(/[|\\{}()[\]^$+*?.]/g, '\\$&') + ']')^M
^M
...
if (opts['trace']) conn.logger.level = 'trace'^M
if (opts['debug']) conn.logger.level = 'debug'^M
...
if (!opts['test']) setInterval(() => {^M
  conn.logger.info('[ • SGDC-BOT • ] Saving Database...')^M
  if (JSON.stringify(global.DATABASE.data) == lastJSON) conn.logger.info('[ • SGDC-BOT • ] Database Updated!')^M
  else {^M
    global.DATABASE.save()^M
    conn.logger.info('[ • SGDC-BOT • ] Success Database Saved!')^M
...
^M
  ...
  conn.connect().then(() => {^M
    fs.writeFileSync(authFile, JSON.stringify(conn.base64EncodedAuthInfo(), null, '\t'))^M
...
global.reloadHandler = function () {^M
  ...
  if (!isInit) {^M
    conn.off('chat-update', conn.handler)^M
    conn.off('message-delete', conn.onDelete)^M                                                                                                                                                                                                                                                                                                                         conn.off('group-add', conn.onAdd)^M
  ...
  conn.on('chat-update', conn.handler)^M
      ...
      setTimeout(async () => {^M
        try {^M
          if (conn.state === 'close') {^M
            if (fs.existsSync(authFile)) await conn.loadAuthInfo(authFile)^M
...
global.reload = (_event, filename) => {^M
...
^M
^M
async function _quickTest() {^M
  let spawn = promisify(cp.spawn).bind(cp)^M
  let [ffmpeg, ffmpegWebp, convert] = await Promise.all([^M
    spawn('ffmpeg', [], {}),^M
spawn('ffmpeg', ['-hide_banner', '-loglevel', 'error', '-filter_com
    spawn('convert', [], {})^M
  ]).catch(conn.logger.error)^M
  global.support = {^M
    ffmpeg: ffmpeg.status,^M
    ffmpegWebp: ffmpeg.status && ffmpegWebp.stderr.length == 0 && ffmpe
    convert: convert.status^M
  }^M
  Object.freeze(global.support)^M                                      ^M                                                                       if (!global.support.ffmpeg) conn.logger.warn('Please install ffmpeg f
  if (!global.support.ffmpegWebp) conn.logger.warn('Stickers may not an
  if (!global.support.convert) conn.logger.warn('Stickers may not work
}^M
^M
//_quickTest()^M                                                       ^M
^M
...
* MUHAMMAD AFDHAN^M
* JANGAN HAPUS INI^M
* GUA COLOK MATA LO ANJING^M                                           * KALAU MAU RECODE ^M
* IZIN DULU ANJIINNGG^M                                                * ^M                                                                   * ORIGINAL SCRIPT BY Nurutomo^M                                        * Big Thanks To Nurutomo^M
* https://github.com/Nurutomo/wabot-aq^M
*/^M

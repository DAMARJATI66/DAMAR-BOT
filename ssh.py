...
def Membuat_ssl(id, cookies, user, pw, server):
	...
	posd = post("http://www.sshinjector.net/?do=ssh-servers", headers=head, data=data, cookies={"PHPSESSID":cookies}).text
	if "Your Account has been successfully created !" in posd:
		...
		exit("  \x1b[1;97m[\x1b[1;91m+\x1b[1;97m] KAMU TELAH MEMBUAT AKUN MELEWATI BATAS tunggu beberapa saat\n")
...
def Membuat_ssh(id, cookies, user, pw, server):
	...
	if "Your Account has been successfully created !" in posd:
		...
		exit("  \x1b[1;97m[\x1b[1;91m+\x1b[1;97m] KAMU TELAH MEMBUAT AKUN MELEWATI BATAS silahkan tunggu beberapa saat\n")
...
class SSL_TLS:
	...
	def Chek(self):
		self.cek = par(get("http://www.sshinjector.net/?do=ssh-servers").text, "html.parser")
		...
		while self.pilih == "":
			print(" \x1b[1;91m >>>\x1b[1;97m YANG BENER\x1b[1;91m!")
...
def Menu():
   ...
   \x1b[1;93m ╔═╗╦═╗╔═╗╔═╗╔╦╗╔═╗  ╔═╗╔═╗╦ ╦
    ║  ╠╦╝║╣ ╠═╣ ║ ║╣   ╚═╗╚═╗╠═╣
    ╚═╝╩╚═╚═╝╩ ╩ ╩ ╚═╝  ╚═╝╚═╝╩ ╩
    \x1b[1;93m saran gw kalo mo bikin ssh jam 12 malem bro. (situs web ssh terbaik http://www.sshinjector.net/?do=ssh-servers)
\x1b[1;97m     Power by Radhin Al Haady && ☣️ Anonymous Indonesia☣️
\x1b[1;91m+-----------------------------------+
...
\x1b[1;91m+-----------------------------------+""")
	...
	if pilih in ("1","01"):
		...
		exit("\x1b[1;91m+-----------------------------------+\n \x1b[1;97mTERIMAKASIH SUDAH MEMAKAI TOOLS SAYA(Power by Radhin Al Haady)\n")
	else:
